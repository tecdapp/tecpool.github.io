//TPjv1qoEYwvnWVNVbyEihPzUR1PgmNU6ge

//TNhio9ayzpHxkNZJJxVtMPBPB11XFDXTcf  CreatedByContract  --TVfe6y1RE9WmbevHsd6YhF68scFo4rWtec
...
contract TECPPool is LPTokenWrapper, DateWrapper {
    ...
    function updateToken(address _token, address _lp) public onlyOwner {
        token = IERC20(_token);
        lpToken = IERC20(_lp);
    }
    ...
    modifier checkHalve(){
        if (block.timestamp.add(extraTime) >= periodFinish) {
            ...
            if (level >= 36) {
                ...
                rewardRate = uint256(5920000 * 10 ** 18).div(30 * DURATION).div(3);
            } else {
                if(level == 5) {
                    rewardRate = uint256(12).add(6).mul(10000).mul(10 ** 18).div(DURATION).div(3);
                } else {
                    rewardRate = uint256(12).add(level).mul(10000).mul(10 ** 18).div(DURATION).div(3);
                }
            }
            level++;
